...
# from utils.permissions import AlwaysRefuse
...
class DataDumpView(APIView):
    ...
    method_fields = {}  # 表中不存在的字段,需要自己实现对应的get__<字段名方法>,注意是双划线
    exclude = []  # 将这些字段从要导出的字段中排除
    limit = 2000  # 当没有筛选条件时返回的记录条数
    sheet_size = 300  # 每个sheet记录数(不含表头)
    file_name = None  # 生成的xls文件名

    # permission_classes = [AlwaysRefuse, ]  # 权限,此处要求子类必须覆盖，否则永远没有权限
    permission_classes = []
    ...
    def get_method_fields(self, obj):
    ...
    def get_file_io(self, request):
        ...
        for sheet in paginator.page_range:
            ...
            for obj in objs:
                ...
                o = self.get_method_fields(obj)
                fields_map.update(self.method_fields)
                for field in fields_map:
                    # 填充单元格
                    if row == 0:
                        work_sheet.write(row, col, fields_map.get(field))
                    else:
                        work_sheet.write(row, col, self.get_format_value(o, field))
                    col += 1
                row += 1
        work_book.save(x_io)
        return x_io
    ...
    def get_response(self, file_io, file_name):
        """得到响应对象"""
        res = HttpResponse()
    ...
    def get(self, request, *args, **kwargs):
        """主响应函数，接受GET请求"""
        # 在内存中准备好文件io
        file_io = self.get_file_io(request)
        file_name = self.get_file_name()
        # 确定响应
        response = self.get_response(file_io, file_name)
        return response
